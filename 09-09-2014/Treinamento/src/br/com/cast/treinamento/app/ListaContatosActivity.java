...
public class ListaContatosActivity extends LifeCicleActivity {
	...
	private void carregaListViewPorFiltro(Contato filtro) {
		List<Contato> contatos = ContatoService.getInstancia(this).listarContatosPorFiltro(filtro);
	...
	protected void onResume() {
		super.onResume();
		filtro = (Contato) getIntent().getSerializableExtra("CHAVE_CONTATO");
		if (!TextUtils.isEmpty(filtro.getNome()) || !TextUtils.isEmpty(filtro.getTelefone())) {
			carregaListViewPorFiltro(filtro);
		} else {
			carregarListView();
		}
	}
