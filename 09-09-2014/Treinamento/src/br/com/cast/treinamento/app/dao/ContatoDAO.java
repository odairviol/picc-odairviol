...
public final class ContatoDAO extends BaseDAO {
	...
	public List<Contato> listarContatosPorFiltro(Contato filtro) {
		SQLiteDatabase db = super.getReadableDatabase();
		try {
			// Opação 2: método db.compileStatement
			String sql = String.format("SELECT * FROM %s WHERE UPPER(%s) = ? OR %s = ? ORDER BY UPPER(%s) DESC", ContatoEntity.TABELA, ContatoEntity.COLUNA_NOME,
					ContatoEntity.COLUNA_TELEFONE, ContatoEntity.COLUNA_NOME);
			String[] argumentosWhere = new String[] { filtro.getNome().toUpperCase(), filtro.getTelefone() };
			Cursor cursorRawQuery = db.rawQuery(sql, argumentosWhere);
			return ContatoEntity.bindContatos(cursorRawQuery);
		} catch (SQLException excecao) {
			Log.e(TAG, excecao.getMessage());
			throw excecao;
		} finally {
